...
class GroupsApi extends Controller
{
    ...
    public function all(Request $request)
    {
        ...
        $GroupsUsers = GroupsUsers::with(['Users', 'Groups'])->orderBy('id', 'DESC')->get();
        ...
        foreach ($nama as $key => $name) {
            ...
            foreach ($usr as $v2) {
                if (isset($v2[str_replace(" ", "_", $name)])) {
                    $username .= $v2[str_replace(" ", "_", $name)] . '<br>';
                }
            }
            ...
            if ($permission[$key]) {
                ...
                foreach ($judul as $v) {
                    $izin .= ' <div class="flex mb-1">' . ucfirst($v) . ' <span class="text-right w-full mr-2"> ';
    ...
    public function view(Request $request)
    {
        ...
        if ($id) {
            $data = [];
            if ($dt = Groups::with('GroupsUsers')->where('id', $id)->first()) {

                $dtUsr = [];
                foreach ($dt['GroupsUsers'] as $key => $value) {
                    if ($user = User::where('id', $value['users_id'])->first()) {
                        $dtUsr[] = ['value' => $value['users_id'], 'label' => $user['username']];
                    }
                }
                $data['id'] = $dt['id'];
                $data['name'] = $dt['name'];
    ...
    public function delete(Request $request)
    {
        ...
        if ($id) {
            if (Groups::where('id', $id)->delete()) {
                GroupsUsers::where('groups_id', $id)->delete();
                return response()->json(
                    [
                        'response' => 'success',
    ...
    public function update(Request $request)
    {
        ...
        if ($id && $name  && $permission && $target) {
            ...
            if ($data = Groups::where('id', $id)->first()) {
    ...
    public function create(Request $request)
    {
        ...
        if ($name && $permission && $target) {
            ...
            if ($validator->fails()) {
                ...
                if (!$targetValid) {
                    if (!Groups::where('name', $request->input('name'))->count()) {

                        if ($id = Groups::insertGetId([
                            'name' => $name,
                            'permission' => serialize($permission),
                            'created_at' => date('Y-m-d H:i:s')
                        ])) {
